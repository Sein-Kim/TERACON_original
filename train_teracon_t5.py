...
def main():
    ...
    parser.add_argument('--savepath',type=str, default='./saved_models/task5')
    parser.add_argument('--datapath', type=str, default='./Data/ColdRec/original_desen_gender.csv',
                        help='data path')
    ...
    parser.add_argument('--paths', type=str, default='./saved_models/task4.t7')
    ...
    task1_dict, task2_dict, task3_dict, task4_dict,task5_dict = task5_model(args)
    model.load_state_dict(task5_dict, strict=False)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_sam in enumerate(getBatch(train_set, batch_size)):
            ...
            _,prev_output1,pmasks1 = model_1(inputs[rand_idx,:-1],s,[], 0,args.gpu_num,onecall=True,current_task=4)
            _,prev_output2,pmasks2 = model_2(inputs[rand_idx2,:],s,[], 1,args.gpu_num,onecall=True,current_task=4)
            _,prev_output3,pmasks3 = model_3(inputs[rand_idx3,:],s,[], 2,args.gpu_num,onecall=True,current_task=4)
            _,prev_output4,pmasks4 = model_4(inputs[rand_idx4,:],s,[], 3,args.gpu_num,onecall=True,current_task=4)


            outputs, _,masks = model(inputs,s,[], 4,args.gpu_num,onecall=True,current_task=4)

            student_out1,_,masks1 = model(inputs[rand_idx,:-1],s,[], 0, args.gpu_num,onecall=True,backward=True,current_task=4)
            student_out2,_,masks2 = model(inputs[rand_idx2,:],s,[], 1, args.gpu_num,onecall=True,backward=True,current_task=4)
            student_out3,_,masks3 = model(inputs[rand_idx3,:],s,[], 2, args.gpu_num,onecall=True,backward=True,current_task=4)
            student_out4,_,masks4 = model(inputs[rand_idx4,:],s,[], 3, args.gpu_num,onecall=True,backward=True,current_task=4)
            ...
            coef_sum = random_batch_size+random_batch_size2+random_batch_size3+random_batch_size4
            ...
            if not torch.isfinite(loss):
                ...
                loss += (args.alpha * (transfer_loss_1*coef1+transfer_loss_2*coef2 + transfer_loss_3*coef3 + transfer_loss_4*coef4))
                loss.backward()
                train_loss += loss.item()
                transfer_loss_t +=args.alpha * (transfer_loss_1.item()*coef1+transfer_loss_2.item()*coef2 + transfer_loss_3.item()*coef3 + transfer_loss_4.item()*coef4)
                thres_cosh = 50
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_sam in enumerate(getBatch(valid_set_, batch_size_test)):
                ...
                outputs, _,masks1 = model(inputs,smax,masks_list, 4,args.gpu_num,onecall=True,current_task=4)
        ...
        if epoch == 0:
            ...
            model_test_acc(model_para, test_set_, model,epoch,args,list__,smax, backward=False,task_num = 4, current_task=4)
        else:
