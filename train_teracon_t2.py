...
def main():
    ...
    parser.add_argument('--datapath', type=str, default='./Data/ColdRec/original_desen_finetune_click_nouserID.csv',
                        help='data path')
    ...
    task1_dict, task2_dict = task2_model(args)
    model.load_state_dict(task2_dict, strict=False)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_sam in enumerate(getBatch(train_set, batch_size)):
            ...
            masks_list = []
            _,prev_output,masks = model_1(inputs[rand_idx,:-1],s,masks_list, 0,args.gpu_num,onecall=True,current_task=1)


            masks_list = []
            outputs, _,masks1 = model(inputs,s,masks_list, 1,args.gpu_num,onecall=True,current_task=1)
            masks_list = []
            student_out,_,masks2 = model(inputs[rand_idx,:-1],s,masks_list, 0, args.gpu_num,onecall=True,backward=True,current_task=1)

            transfer_loss = criterion_(student_out,prev_output)
            ...
            for i in range(len(masks)):
                jaccard +=gate(6*cos(masks[i],masks1[i])).detach().cpu().item()
                len_jaccard+=1
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_sam in enumerate(getBatch(valid_set_, batch_size_test)):
                ...
                outputs, _,masks1 = model(inputs,smax,masks_list, 1,args.gpu_num,onecall=True,current_task=1)
        ...
        if epoch == 0:
            ...
            model_test(model_para, test_set_, model,epoch,args,list__,smax,backward=False,task_num=1,current_task=1)
        else:
