...
import TERACON_Past.teracon as generator_recsys
...
def main():
    ...
    parser.add_argument('--savepath',type=str, default='./saved_models/task4')
    parser.add_argument('--datapath', type=str, default='./Data/ColdRec/original_desen_age.csv',
                        help='data path')
    ...
    parser.add_argument('--paths', type=str, default='./saved_models/task3.t7')
    ...
    task1_dict, task2_dict, task3_dict, task4_dict = task4_model(args)
    model.load_state_dict(task4_dict, strict=False)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_sam in enumerate(getBatch(train_set, batch_size)):
            ...
            _,prev_output1,pmasks1 = model_1(inputs[rand_idx,:-1],s,[], 0,args.gpu_num,onecall=True,current_task=3)
            _,prev_output2,pmasks2 = model_2(inputs[rand_idx2,:],s,[], 1,args.gpu_num,onecall=True,current_task=3)
            _,prev_output3,pmasks3 = model_3(inputs[rand_idx3,:],s,[], 2,args.gpu_num,onecall=True,current_task=3)


            outputs, _,masks = model(inputs,s,[], 3,args.gpu_num,onecall=True,current_task=3)

            student_out1,_,masks1 = model(inputs[rand_idx,:-1],s,[], 0, args.gpu_num,onecall=True,backward=True,current_task=3)
            student_out2,_,masks2 = model(inputs[rand_idx2,:],s,[], 1, args.gpu_num,onecall=True,backward=True,current_task=3)
            student_out3,_,masks3 = model(inputs[rand_idx3,:],s,[], 2, args.gpu_num,onecall=True,backward=True,current_task=3)
            ...
            if not torch.isfinite(loss):
                ...
                loss += (args.alpha* (transfer_loss_1*(random_batch_size/(random_batch_size+random_batch_size2+random_batch_size3))+transfer_loss_2*(random_batch_size2/(random_batch_size+random_batch_size2+random_batch_size3))+transfer_loss_3*(random_batch_size3/(random_batch_size+random_batch_size2+random_batch_size3))))
                loss.backward()
                train_loss += loss.item()
                transfer_loss_t+=args.alpha* (transfer_loss_1.item()*(random_batch_size/(random_batch_size+random_batch_size2+random_batch_size3))+transfer_loss_2.item()*(random_batch_size2/(random_batch_size+random_batch_size2+random_batch_size3))+transfer_loss_3.item()*(random_batch_size3/(random_batch_size+random_batch_size2+random_batch_size3)))
                thres_cosh = 50
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_sam in enumerate(getBatch(valid_set_, batch_size_test)):
                ...
                outputs, _,masks1 = model(inputs,smax,masks_list, 3,args.gpu_num,onecall=True,current_task=3)
        ...
        if epoch == 0:
            ...
            model_test_acc(model_para, test_set_, model,epoch,args,list__,smax, backward=False,task_num = 3, current_task=3)
        else:
